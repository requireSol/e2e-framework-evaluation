...
describe("actions", () => {
  ...
  // this test was automatically generated from cypress and modified from enam solaimani
  // this fits our needs for action test and we can test all the functionality

  it(".type() - type into a DOM element", () => {
    // https://on.cypress.io/type
    cy.getBySel("input_type").type("fake@email.com");
    cy.getBySel("text_type").should("have.value", "fake@email.com");

    // .type() with special character sequences
    cy.getBySel("input_type").type("{leftarrow}{rightarrow}{uparrow}{downarrow}");
    cy.getBySel("input_type").type("{del}{selectall}{backspace}");

    // .type() with key modifiers
    cy.getBySel("input_type").type("{alt}{option}"); //these are equivalent
    cy.getBySel("input_type").type("{ctrl}{control}"); //these are equivalent
    cy.getBySel("input_type").type("{meta}{command}{cmd}"); //these are equivalent
    cy.getBySel("input_type").type("{shift}");

    // Delay each keypress by 0.1 sec
    cy.getBySel("input_type").type("slow.typing@email.com", { delay: 100 });
    cy.getBySel("text_type").should("have.value", "slow.typing@email.com");
  });

  it(".blur() - blur off a DOM element", () => {
    // https://on.cypress.io/blur
    cy.getBySel("input_blur").type("About to blur");
    cy.getBySel("input_blur").blur();
    cy.getBySel("input_blur")
  ...
  it(".clear() - clears an input or textarea element", () => {
    ...
    cy.getBySel("input_clear").type("Clear this text");
    cy.getBySel("text_clear").should("have.value", "Clear this text");
    cy.getBySel("input_clear").clear();
    cy.getBySel("text_clear").should("have.value", "");
  });
